...
export class RouletteResultProcessor {
  ...
  /**
   * Process VRF value to generate roulette number and color
  ...
  processVRF(vrfValue, gameConfig) {
    try {
      ...
      const number = this.generateRouletteNumber(vrfBigInt);
      ...
      console.error('❌ Failed to process Roulette VRF:', error);
      throw new Error(`Roulette VRF processing failed: ${error.message}`);
    }
  }

  /**
   * Generate roulette number using VRF value
   * @param {bigint} vrfValue - VRF random value
   * @returns {number} Roulette number (0-36)
   */
  generateRouletteNumber(vrfValue) {
    // Use modulo operation to get number in range 0-36
    const number = Number(vrfValue % BigInt(this.totalNumbers));
    return number;
  }
