...
export class WheelResultProcessor {
  ...
  /**
   * Process VRF value to generate wheel segment position
  ...
  processVRF(vrfValue, gameConfig) {
    try {
      ...
      const segment = this.generateWheelSegment(vrfBigInt, segments);
      ...
      console.error('❌ Failed to process Wheel VRF:', error);
      throw new Error(`Wheel VRF processing failed: ${error.message}`);
    }
  }

  /**
   * Generate wheel segment using VRF value
   * @param {bigint} vrfValue - VRF random value
   * @param {number} segments - Number of segments
   * @returns {number} Segment number (0 to segments-1)
   */
  generateWheelSegment(vrfValue, segments) {
    // Use modulo operation to get segment in range 0 to segments-1
    const segment = Number(vrfValue % BigInt(segments));
    return segment;
  }
