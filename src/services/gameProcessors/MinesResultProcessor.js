...
export class MinesResultProcessor {
  ...
  /**
   * Process VRF value to generate mine positions
   * @param {string|bigint} vrfValue - VRF random value
   * @param {Object} gameConfig - Game configuration
   * @returns {Object} Mines game result
   */
  processVRF(vrfValue, gameConfig) {
    try {
      ...
      // Convert VRF value to BigInt for processing
      ...
      const minePositions = this.generateMinePositions(vrfBigInt, mineCount);
      ...
      console.error('❌ Failed to process Mines VRF:', error);
      throw new Error(`Mines VRF processing failed: ${error.message}`);
    }
  }

  /**
   * Generate mine positions using Fisher-Yates shuffle algorithm
   * @param {bigint} vrfValue - VRF random value
   * @param {number} mineCount - Number of mines to place
   * @returns {number[]} Array of mine positions
   */
  generateMinePositions(vrfValue, mineCount) {
  ...
  /**
   * Validate mine positions against VRF value
   ...
   */
  validateMinePositions(vrfValue, minePositions, mineCount) {
    try {
      // Regenerate positions using the same VRF value
      const vrfBigInt = typeof vrfValue === 'string' ? BigInt(vrfValue) : vrfValue;
      const regeneratedPositions = this.generateMinePositions(vrfBigInt, mineCount);
